...
from codex_sdk_cli.api.use_case_dependencies.codex_usage import ListCodexUsageUseCaseDep
from codex_sdk_cli.domains.codex_usage.schemas import (
...
@router.get("/codex-usage/by-job", response_model=CodexUsageByJobResponse)
async def list_codex_usage_by_job(
    use_case: ListCodexUsageUseCaseDep,
    source: str | None = Query(default=None, min_length=1, max_length=128),
    status: str | None = Query(default=None, min_length=1, max_length=32),
    model: str | None = Query(default=None, min_length=1, max_length=128),
    ...
    return await use_case.execute_by_job(
        source=source,
        status=status,
        model=model,
        reasoning_effort=reasoning_effort,
