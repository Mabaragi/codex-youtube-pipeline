...
- `display_summary`는 밋밋한 감상 한 줄보다 구체적인 장면, 반응, 상황의 반전을 우선한다. 단, 입력에 없는 감정이나 사건은 만들지 않는다.
- micro-event나 cue 흐름에서 우는 상황, 울상, 상처받은 반응, 삐진 리액션이 핵심이면 title/summary/display copy에 그 장면 가치를 살린다.
- 우는 장면이 episode의 핵심이면 `display_title`에도 울음, 눈물, 울상 같은 탐색 가능한 표현을 넣는다.
- 우는 장면이 있으면 반드시 관련 `topic_cluster`에 포함하고, 주요 흐름 목록에서 찾을 수 있도록 `display_label`이나 `summary`에 그 장면을 드러낸다.
- `display_summary`는 내부 summary를 말투만 바꾼 보고문으로 만들지 않는다.
- `display_summary`는 `A가 B한다`, `A가 B를 진행한다`, `A로 이어진다`, `구간이다`, `확인한다`, `방송이다` 같은 보고서형 종결과 문장 구조를 피한다.
- `display_summary`는 특정 어미나 종결 패턴을 정답처럼 반복하지 않는다. 같은 영상 안에서 비슷한 끝맺음과 나열 구조가 반복되면 실패로 본다.
- `display_summary`는 짧은 팬 반응형 설명으로 쓴다. 피드에서 보고 누르고 싶게, 가볍고 귀엽고 장면감 있게 쓴다.
