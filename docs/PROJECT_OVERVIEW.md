...
object name, URI, response hash, 언어/segment 수 같은 메타데이터를 저장한다.
...
- `tests/test_youtube_repository.py`: YouTube transcript metadata repository insert/update를 검증한다.
...
현재 initial revision은 `youtube_transcripts` 테이블을 생성한다.
