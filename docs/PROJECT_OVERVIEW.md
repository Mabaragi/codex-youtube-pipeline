...
- `POST /pipeline/jobs/{id}/retry`: 실패한 pipeline job을 같은 job 아래 새 attempt로 재시도한다. 현재는 `channel_resolve`와 `video_collect`를 지원한다.
...
- `POST /channels/{id}/videos/collect`: local channel row의 YouTube channel ID로 `search.list` 최신순 수집을 실행하고, 기존 video를 만나면 중단하며, `videos.list` 상세 metadata와 raw API call metadata를 남긴다.
...
`pipeline_jobs`, `pipeline_job_attempts`, `videos`, `video_tasks` 테이블을 사용한다.
...
`external_api_calls`, `pipeline_jobs`, `pipeline_job_attempts`, `videos`, `video_tasks` 테이블과
