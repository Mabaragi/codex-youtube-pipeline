...
flowchart TD
    ...
    HomeRunner --> PullImages["docker compose pull api codex ops-ui"]
    PullImages --> Migration["docker compose run api alembic upgrade head"]
    Migration --> ComposeDeploy["docker compose up -d --no-build api ops-ui nginx ngrok minio"]
...
sequenceDiagram
    ...
    Runner->>Docker: docker compose pull api codex ops-ui
    Runner->>Docker: docker compose run api alembic upgrade head
    Runner->>Docker: docker compose up -d --no-build api ops-ui nginx ngrok minio
...
6. `api`, `codex`, `ops-ui` image를 GHCR에서 pull하고 같은 compose env로 `alembic upgrade head`를 실행한다.
7. `api`, `ops-ui`, `nginx`, `ngrok`, `minio`를 `up -d --no-build --remove-orphans`로 배포한다.
