...
flowchart TD
    ...
    Migration --> ComposeDeploy["docker compose up -d --build --force-recreate api ops-ui nginx ngrok minio"]
...
sequenceDiagram
    ...
    Runner->>Docker: docker compose up -d --build --force-recreate api ops-ui nginx ngrok minio
...
7. `api`, `ops-ui`, `nginx`, `ngrok`, `minio`를 `up -d --build --remove-orphans`로 배포한다.
