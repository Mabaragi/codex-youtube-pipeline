...
- `docs/YOUTUBE_DATA_PIPELINE_TODO.md`: YouTube channel resolve, raw response 저장, videos/transcripts/LLM summary 파이프라인의 계획과 구현 TODO를 정리한다.
- `docs/erd.svg`: SQLAlchemy `Base.metadata`에서 생성한 현재 데이터베이스 ERD다. DB 모델이나 Alembic migration을 바꾼 뒤 `uv run python scripts/generate_erd.py`로 갱신한다.
