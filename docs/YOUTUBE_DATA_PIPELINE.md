...
- `GET /pipeline/jobs/{jobId}`: 단일 job의 입력, attempts, 연결된
  `external_api_calls`, 현재 구현된 domain output인 `channels`, `videos`를 함께 반환한다.
- `POST /pipeline/jobs/{jobId}/retry`: failed job 재시도 command다. 현재는
  `channel_resolve`와 `video_collect`를 지원한다.
...
`POST /pipeline/jobs/{jobId}/retry`는 현재 `channel_resolve`와 `video_collect`
