...
이 문서는 YouTube channel resolve부터 videos, transcripts, cues, micro-events,
timeline compose, archive publish까지 이어지는 데이터 파이프라인의 공통 설계 원칙을
정리한다. 특정 endpoint 구현 절차보다, 앞으로 새 수집/가공 단계를 추가할 때 따라야
하는 상태 추적과 데이터 연결 규칙을 우선한다.
...
8. 선택한 timeline을 archive artifact와 index로 publish한다.
9. 작업 이력, operation events, Codex usage를 운영 UI/API에서 조회한다.

각 단계는 동기 REST API, 수동 실행, DB polling worker queue, 또는 local scheduler
process로 실행될 수 있다. 실행 방식이 달라도 job/attempt/raw/domain row 연결
규칙은 동일해야 한다.
...
- `POST /pipeline/jobs/{jobId}/retry`: failed job 재시도 command다. 현재
  ...
  `transcript_cue_generate`, `micro_event_extract`, `timeline_compose`,
  `archive_publish`를 지원한다.
...
`codex-pipeline-scheduler` is the local periodic coordinator for the early
pipeline. It is a separate Windows process, not a loop inside FastAPI. The
local native start/deploy scripts run it next to the API, micro-event worker,
timeline worker, and Ops UI.

The scheduler tick does four bounded actions:

1. List all local `channels` that have `youtube_channel_id`.
2. For each channel, run `video_collect` only when there is no running
   `video_collect` job and the latest succeeded `video_collect` is older than
   `CODEX_CLI_PIPELINE_SCHEDULER_CHANNEL_INTERVAL_SECONDS`.
3. After video collection, run channel transcript collection with
   `CODEX_CLI_PIPELINE_SCHEDULER_TRANSCRIPT_LIMIT`.
4. Separately recheck old `video_tasks.status="no_transcript"` rows with
   `recheckNoTranscript=true` when their `completed_at` is older than
   `CODEX_CLI_PIPELINE_SCHEDULER_NO_TRANSCRIPT_RECHECK_INTERVAL_SECONDS`.

Scheduler-triggered work passes `actorType="system"` to operation events. It
uses the same video collect and transcript collect use cases as the manual APIs,
so `pipeline_jobs`, `pipeline_job_attempts`, `video_tasks`, raw API call records,
and transcript/cue outputs remain the source of truth. The scheduler does not
create a separate schedule table in v1; due checks are derived from existing
job/task timestamps.

Default settings:

- `CODEX_CLI_PIPELINE_SCHEDULER_ENABLED=true`
- `CODEX_CLI_PIPELINE_SCHEDULER_POLL_INTERVAL_SECONDS=300`
- `CODEX_CLI_PIPELINE_SCHEDULER_CHANNEL_INTERVAL_SECONDS=86400`
- `CODEX_CLI_PIPELINE_SCHEDULER_TRANSCRIPT_LIMIT=5`
- `CODEX_CLI_PIPELINE_SCHEDULER_NO_TRANSCRIPT_RECHECK_INTERVAL_SECONDS=604800`
- `CODEX_CLI_PIPELINE_SCHEDULER_NO_TRANSCRIPT_LIMIT=2`
- `CODEX_CLI_PIPELINE_SCHEDULER_ID`: optional worker id override.

Scheduler가 enabled인 경우 `CODEX_CLI_YOUTUBE_DATA_API_KEY`가 필요하다. YouTube
metadata 수집을 하지 않는 local runtime은 key를 비워 두는 대신
`CODEX_CLI_PIPELINE_SCHEDULER_ENABLED=false`로 scheduler를 끈다.
...
`timeline_compose`, `archive_publish` failed job을 지원한다. retry는 기존 job을
덮어쓰지 않고 같은 job 아래 새 attempt를 만든다. `succeeded`, `running`, `skipped`,
`canceled` job은 retry하지 않는다. 입력이 달라져야 하는 재실행은 retry가 아니라 새
job으로 처리한다.
